...
- Use DuckDB (embedded) for fundamentals and economic indicators — benchmark winner across all analytical workloads (screening: 34ms, complex: 96ms). Top alternatives if DuckDB is unsuitable: **SQLite** (35ms screening, zero-config embedded), **PostgreSQL** (38ms screening, already in the stack), **ClickHouse** (45ms screening / 185ms complex, best server-side columnar option). See [fundamentals_database.md](fundamentals_database.md) for full benchmark results.
