...
| [backend_server.md](backend_server.md) | Backend server decision — Gunicorn+Uvicorn Raw ASGI as the default, with full benchmark data from both Server Benchmarks and Extended Server Benchmarks runs; FastAPI on Gunicorn+Uvicorn only when WebSocket or auto-docs are required |
