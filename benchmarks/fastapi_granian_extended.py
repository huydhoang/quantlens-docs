...
"""
Stack 2: Granian + FastAPI + NautilusTrader + skfolio
Using Granian (Rust-based) server with FastAPI framework
"""
...
async def lifespan(app: FastAPI):
    """Granian-compatible lifespan"""
    config = TradingNodeConfig(
        trader_id='GRANIAN-FASTAPI-001',
    ...
    print('✅ Granian + FastAPI + NautilusTrader + skfolio initialized')
    yield
    if state.kernel:
...
app = FastAPI(
    title='Granian + FastAPI Trading API',
    lifespan=lifespan
)
