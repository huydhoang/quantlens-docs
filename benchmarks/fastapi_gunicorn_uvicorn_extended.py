...
"""
Stack 6: Gunicorn + Uvicorn Workers + FastAPI + NautilusTrader + skfolio
Multi-process FastAPI using Gunicorn with Uvicorn worker class
"""
...
# Multi-process state - each Gunicorn worker has isolated state
class ProcessState:
    def __init__(self):
...
async def lifespan(app: FastAPI):
    ...
    Each Gunicorn worker initializes its own NautilusTrader instance
    """
    state.worker_id = os.getpid()

    config = TradingNodeConfig(
        trader_id=f'GUNICORN-FASTAPI-WORKER-{state.worker_id}',
    ...
    print(f'✅ Worker {state.worker_id} initialized: Gunicorn + Uvicorn + FastAPI + NautilusTrader + skfolio')

    yield

    if state.kernel:
        await state.kernel.cleanup()
    print(f'🛑 Worker {state.worker_id} shutdown complete')

app = FastAPI(
    title='Gunicorn + Uvicorn Workers + FastAPI Trading API',
    lifespan=lifespan
)
...
# Health check - useful for load balancer
@app.get('/health')
...
async def optimize_portfolio(request: PortfolioRequest):
    """
    Optimize portfolio using skfolio MeanRisk model
    Runs in worker process - results not shared between workers
    """
    try:
        ...
        return {
            ...
            'worker_pid': state.worker_id  # Debug: show which worker processed
        }
...
async def hierarchical_portfolio(request: PortfolioRequest):
    """Hierarchical Risk Parity optimization using skfolio"""
    prices = load_sp500_dataset()
    available = [s for s in request.symbols if s in prices.columns]
    prices = prices[available].dropna()
    X = prices_to_returns(prices)
...
async def submit_order(
    ...
    """
    Submit order via NautilusTrader execution client
    NOTE: Orders are isolated to this worker process
    """
    try:
...
async def get_positions(trader: Trader = Depends(get_trader)):
    """
    Get current positions from NautilusTrader cache
    NOTE: Positions are per-worker; use external cache for shared state
    """
...
async def run_backtest(
    ...
    """
    Run backtest asynchronously using NautilusTrader
    Results stored in worker-local cache (not shared)
    """
    async def backtest_task():
        ...
        try:
            bt_config = BacktestEngineConfig(
                trader_id=f'BACKTEST-{request.strategy_name}-{state.worker_id}',
                logging=LoggingConfig(log_level='INFO')
            )
    ...
    return {'status': 'started', 'strategy': request.strategy_name, 'worker_pid': state.worker_id}
...
async def get_backtest_results(strategy_name: str):
    ...
    result['worker_pid'] = state.worker_id
    return result
